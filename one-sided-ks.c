...
static double invert_threshold(size_t min_count, double target, bool up,
    double threshold(double x, double log_b),
    double log_b)
{
	...
	for (size_t i = 0; i < 64; ++i) {
		...
		const double pivot = bits_float(low_bits + (high_bits - low_bits) / 2);
 ...
 */
static double invert_threshold_up(double target, size_t min_count, double log_eps)
{
	return invert_threshold(min_count, target, true,
	    threshold_up, log_b_up(min_count, log_eps));
}

/*
 * Under approximate g(target).
 */
static double invert_threshold_down(double target, size_t min_count, double log_eps)
{
	return invert_threshold(min_count, target, false,
	    threshold_down, log_b_down(min_count, log_eps));
}
...
double one_sided_ks_expected_iter(size_t min_count, double log_eps, double delta)
{
	...
	/*
         ...
         */
	const double first_threshold = one_sided_ks_threshold(min_count, min_count, log_eps);
	if (delta > first_threshold / 2) {
		delta = prev(first_threshold / 2);
	}

	const double g_delta = invert_threshold_down(delta, min_count, log_eps);
